...
export const ABILITIES = [
  // — Ship —
  { id: 'elusive',        name: 'Elusive',        category: 'ship', effect: '\u20131 Gunnery to all enemies firing at this ship.' },
  { id: 'flush-deck',     name: 'Flush Deck',     category: 'ship', effect: '+2 to tests to extinguish Fire criticals.' },
  { id: 'handy',          name: 'Handy',          category: 'ship', effect: 'Reroll failed Tacking tests. Must keep the new result.' },
  { id: 'sea-kindly',     name: 'Sea Kindly',     category: 'ship', effect: 'Treat weather one step calmer for movement purposes.' },
  { id: 'stout-hull',     name: 'Stout Hull',     category: 'ship', effect: 'Ignore the first Hull critical each game.' },
  { id: 'tumblehome',     name: 'Tumblehome',     category: 'ship', effect: 'Enemy boarding tests suffer \u20132 Close Action against this ship.' },

  // — Captain —
  { id: 'calculated-ambition', name: 'Calculated Ambition', category: 'captain', effect: 'If the turn ends with no enemy within 6", gain +1 Command next turn.' },
  { id: 'corsair',             name: 'Corsair',             category: 'captain', effect: '+1 Command for boarding and prize-taking. +1 Seamanship when disengaging.' },
  { id: 'crowd-on-sail',       name: 'Crowd On Sail',       category: 'captain', effect: 'Gain +1" movement for two turns, but suffer \u20131 Command during that period.' },
  { id: 'cunning-maneuver',    name: 'Cunning Maneuver',    category: 'captain', effect: 'Once per game, make one free turn before firing a broadside.' },
  { id: 'damn-the-risk',       name: 'Damn the Risk!',      category: 'captain', effect: '+1 Gunnery when firing within 1" of the target.' },
  { id: 'devils-luck',         name: "Devil's Luck",        category: 'captain', effect: 'Force a reroll on three dice per game (yours or opponent\'s).' },
  { id: 'flash-of-brilliance', name: 'Flash of Brilliance', category: 'captain', effect: 'Once per game, roll twice for any test and keep the better result.' },
  { id: 'follow-me',           name: 'Follow Me!',          category: 'captain', effect: 'Friendly ships within 6" gain +1 Command.' },
  { id: 'fox-of-the-sea',      name: 'Fox of the Sea',      category: 'captain', effect: 'If this ship did not use all its maneuvers, it may make one after all other ships have moved.' },
  { id: 'hold-your-fire',      name: 'Hold Your Fire',      category: 'captain', effect: 'Skip firing for a full turn to gain +2 Gunnery next broadside.' },
  { id: 'inspiring-leader',    name: 'Inspiring Leader',    category: 'captain', effect: '+1 to Command tests to Rally.' },
  { id: 'local-pilot',         name: 'Local Pilot',         category: 'captain', effect: 'Reroll all checks to run aground, strike reefs, or hit rocks.' },
  { id: 'no-retreat',          name: 'No Retreat',          category: 'captain', effect: 'Ignore the first failed morale test (Strike or Rally) each game.' },
  { id: 'point-of-honor',      name: 'Point of Honor',      category: 'captain', effect: 'When fighting a single opponent, gain +1 Gunnery if this ship took damage this turn.' },
  { id: 'press-home',          name: 'Press Home',          category: 'captain', effect: '+1 Command when within 1" of an enemy ship.' },
  { id: 'sails-before-guns',   name: 'Sails Before Guns',   category: 'captain', effect: 'If this ship does not fire this round, it may make one extra maneuver.' },
  { id: 'sang-froid',          name: 'Sang-Froid',          category: 'captain', effect: 'Only suffer \u20131 Command when Unsteady (instead of normal penalty).' },
  { id: 'sea-wolf',            name: 'Sea Wolf',            category: 'captain', effect: '+1 Command when within 6" of an enemy flagship.' },
  { id: 'strategist',          name: 'Strategist',          category: 'captain', effect: "Once per game, choose this ship's place in the activation order." },

  // — Crew —
  { id: 'blood-up',            name: 'Blood Up',            category: 'crew', effect: 'After successfully rallying, gain +1 Gunnery.' },
  { id: 'dead-eye',            name: 'Dead Eye',            category: 'crew', effect: '+1 Gunnery at long range.' },
  { id: 'rolling-broadside',   name: 'Rolling Broadside',   category: 'crew', effect: 'Once per game, fire a broadside twice in the same turn.' },
  { id: 'sharpshooters',       name: 'Sharpshooters',       category: 'crew', effect: 'After firing a broadside at short range, make a Close Action test. On success, inflict one additional Officer critical.' },
  { id: 'broadside',           name: 'Broadside!',          category: 'crew', effect: 'Once per game, when firing a broadside, reroll all failed Gunnery dice. Must keep the new results.' },
  { id: 'drilled-gunnery',     name: 'Drilled Gunnery',     category: 'crew', effect: 'Reroll natural 20s on Gunnery tests (normally automatic misses). Must keep the new results.' },
  { id: 'boarders-away',       name: 'Boarders Away!',      category: 'crew', effect: '+1 Close Action in boarding combat.' },
  { id: 'fire-eaters',         name: 'Fire-Eaters',         category: 'crew', effect: '+2 Close Action when this ship initiates boarding.' },
  { id: 'unflinching',         name: 'Unflinching',         category: 'crew', effect: 'No penalty for being Unsteady.' },
  { id: 'stout-hearts',        name: 'Stout Hearts',        category: 'crew', effect: 'Reroll failed Rally tests. Must keep the new result.' },
  { id: 'stand-by-the-captain', name: 'Stand by the Captain', category: 'crew', effect: 'Ignore the first Officer critical each game.' },
  { id: 'jury-rig',            name: 'Jury-Rig',            category: 'crew', effect: 'Once per game, immediately repair one Rigging or Mast critical.' },
  { id: 'bosuns-pride',        name: "Bosun's Pride",       category: 'crew', effect: '+1 to Command tests to repair damage.' },
  { id: 'boarding-nets',       name: 'Boarding Nets',       category: 'crew', effect: 'When the enemy initiates boarding, make a Seamanship test. On success, the boarding is repelled \u2014 the enemy must try again next turn.' },
  { id: 'surgeon',             name: 'Surgeon',             category: 'crew', effect: 'Once per game, test Command. On success, recover one Officer critical or 1 Crew damage.' },

  // — Weakness —
  { id: 'crank',           name: 'Crank',           category: 'weakness', effect: 'In Heavy weather or above, \u20131 Maneuver and \u20131 to Seamanship tests.' },
  { id: 'dry-as-tinder',   name: 'Dry as Tinder',   category: 'weakness', effect: 'After taking a Fire critical, test Command. If failed, take a second Fire critical immediately.' },
  { id: 'faint-heart',     name: 'Faint Heart',     category: 'weakness', effect: 'Must pass a Command test to initiate boarding or close within short range of an enemy.' },
  { id: 'green-crew',      name: 'Green Crew',      category: 'weakness', effect: 'First time this ship takes broadside damage, test Command. If failed, become Unsteady immediately. On natural 1, gain +1 Morale for the rest of the game.' },
  { id: 'hot-headed',      name: 'Hot-Headed',      category: 'weakness', effect: 'At the start of activation, if an enemy is within 12", test Command. If failed, must move toward the nearest enemy this turn.' },
  { id: 'nerves-of-glass', name: 'Nerves of Glass', category: 'weakness', effect: 'After taking broadside damage, test Command. If failed, make a random turn.' },
  { id: 'powder-keg',      name: 'Powder Keg',      category: 'weakness', effect: 'When this ship suffers a Magazine result on the Hull critical table, the explosion occurs even if there is no active Fire critical.' },
  { id: 'rotten-timbers',  name: 'Rotten Timbers',  category: 'weakness', effect: 'When this ship takes a Hull critical, test Seamanship. If failed, take one additional Hull critical.' },
  { id: 'wanting-hands',   name: 'Wanting Hands',   category: 'weakness', effect: 'Cannot fire both broadsides in the same turn. Cannot initiate boarding.' },
];
