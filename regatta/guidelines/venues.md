...
**They collide — `hard`, `contactR: 13`, `wash: 0.48`.** When this was written the
hidden collider was a circle, which is why every vessel in the harbour is `contact: none`:
a 4.3:1 cargo hull sized to its beam left two thirds of the ship sailable-through, sized to
its length it became an invisible wall. **Since 2026-09-14 a hard prop collides on its
actual boundary**: `art/prop_outlines.py` traces each contact kind's bake (inside its
`srcBox` where it has one) into `js/prop_outlines.js`, and `VenueDoc.propHitRings` rotates,
scales and places that outline — the compile, the bots' router, the editor's bake and
test_sailable all read the one function. EVERY kind with a bake is traced (2026-09-14, night),
not only the kinds whose row says hard — a designer can flip one PLACEMENT to `contact: hard`
in the editor (Otter's anemone pool and surfgrass channel are rocks with life on them, placed
hard on the shore), and before this that placement fell back to a circle of r97 round a 180-unit
slab. The rock defines the edge whichever row or placement made it solid. `contactR` is now
the FALLBACK: a composite tree (whose trunk part is traced instead of its crown), a drifting
kind, or a placement that authored its own radius keeps the 12-gon. A buoy is a disc either way, so
nothing changed here; a pier stops a hull along its whole length now, and a rock at the
rock's edge. Hard props standing on the water's surface also take SURF like a shore
(`propSurf`), unless they carry their own `wash` — on the faces that meet the seas only:
a prop gets no refraction floor, because a coast's floor (the swell wrapping into bays)
put a crest on every edge of a small rock at one stand-off and read as a halo. A hard kind
may author a `height` (metres) and the compile hands it to the hidden collider, so the
prop casts the same lee a shape of that height would (see §wind shadow). In the editor's
props mode the selection traces the PAINTED silhouette and a hard prop shows its collider
as a dashed red outline (both from `js/prop_outlines.js`, which carries a `paint` ring for
every kind) — so what you see selected is what the hull will hit, not the sprite's frame.

**A hull can sail UNDER an arch.** An arch kind is a composite like the bayou trees:
`parts.surface` is the PIERS (hard, traced — the collider, drawn under the fleet) and
`parts.canopy` is the SPAN (the top of the arch, drawn over the fleet, `opaque` so it never
fades the way a crown does — the point is NOT seeing your hull for a moment). `art/archsplit.py`
cuts the whole rock's BAKE (not the master: the bake carries the fillTo frame the editor and
the paint silhouette use) into the two parts along the neck — the narrowest cross-section on
the rock's long axis — with a hard seam, so the halves reassemble exactly. The neck's run along
the axis is the passage WIDTH (a hull goes through lengthwise, so its beam is what has to fit),
and the neck's thickness is how far it travels under rock. The part kinds carry `part: true`
and the picker hides them: you place the whole arch. Re-run archsplit.py, then
prop_outlines.py, after any re-ingest of the whole rock. `wash` is r99 ÷ world, by the cypress
...
**Status.** REBUILT 2026-09-13 by `art/build_otter.py` from Wes's coastal-course brief: a one-way hook — sheltered start in the lee of a spur, beat up a granite coast to Otter Point (two rocks and two awash washes give inside / middle / outside), rounding mark just beyond the point, a reach down a coast of five points and coves that bears away 100 → 160 TWA, a rock gate (inside channel with a kelp tax, or outside), and a finish in a bay behind an islet. WIND REWORKED 2026-09-14 by `art/otter_wind.js` (it rewrites `wind.regions` and nothing else — run it after any coastline change, never `build_otter.py`): base 325° FROM at 17.5 kt offshore, twenty regions cut as an exact partition with polygon-clipping (overlapping regions AVERAGE and unstated water is calm, so a hole cannot be laid over a base; each piece is buffered on the wide-falloff side of every seam so mismatched falloffs never leak calm — a probe of the mean field found 0 of 17k water cells under weight 0.98 after the fix). BEAT: start 11.5 at the boat end in the spur's lee and 14 at the pin (the boat end is 350u closer to the mark, so the start is a real choice), inshore 14 @ 318° (backed — the along-shore board is port, and a back lifts it), mid 15 @ 322°, SW offshore 16.5, approach 18.5 @ 320°, headland 20 @ 313°. REACH: coast band 16 from the shore to 450u off the points with three cove lees (first cove 12.5, the cove behind the north point 10.5, the big bight before the gate 8.5) and an 18 kt squeeze off the north point; a 20 kt PRESSURE LANE 450–1400u off the points; 17.5 beyond. The lane is much closer than the brief's sketch because the coast bulges north between the mark and the gate, so a line D further out costs ~2.1 × D of distance and a lane 3000u out can never pay. BAY: mouth 13 (the 325u entrance gap west of the islet stays in pressure), a 6 kt hole over the whole west half, a 14 kt finger down the islet's west face to the east end of the line, 13.5 east of the islet; shifty (dirVar up to 15°). Priced on the mean field (scratch pricer, polar + kelp drag + lees, 6 s a tack): reach — hug the coves 100 s, ride just outside the points 85 s, the lane 900u out 95 s, so the pressure-reading line wins and both extremes lose; bay — corner cut to the west end 21 s, finger to the east end 22, aim at the centre 23, east of the islet 27; beat — the one-tack far-offshore board wins by ~24 s over every short-tacking lane, which is the geometry of a dead beat along a shore that blocks half the cone (the tack count, not the wind), so the beat's choice is the start end plus 'bang the corner or play the middle'. The rocks keep their engine lees (Wes flipped them hard with heights) — small and local, as the brief asked. OBJECT LAYER REWORKED 2026-09-14 against sixteen aerials of Point Lobos, Point Pinos, Pebble Beach and Carmel Highlands, with seven kinds (coastalgranite, tidepool, sunkenrock, kelp, shallows, coastalmeadow, cypressfloor) plus buffsand: the coast is SCALLOPED — one to three round coves and a finger per macro edge, generated once in edge-parameter space and shared by every contour so the granite, meadow, forest and wet-rock rings nest (a bite never reaches an anchor: the first cut let one pull Q3's anchor inland and the finger's granite core floated in open water where the router threads, and test:sailable grounded); the land is FOREST with meadow flats bowed into its edge at the tip and every point, and forest to the rock in the sheltered coves and the bay; the granite is a ragged band of very uneven width (400–460u at the points, 110–160u at cove heads); the wet rock is a fringe (120–160u at points, 35u elsewhere) plus seaward fingers and detached reefs, every one with a pale GRANITE CORE so the pale rock is the body and the dark rock the waterline; every point ends in a GROUP of rock on its own grain (finger or reef, a ridge islet, a stack, a sunken rock, one turquoise halo) and a rock stands in every cove mouth; kelp is fifteen small streaks; BEACHES are lenses whose seaward edge is the coast itself (concave, wall to wall, meeting the water with the apron hidden) at coves 1, 3, 4, 5, the start bight, and a 720u crescent at the bay head. Offset rings fold at sharp corners, so every contour is untangled (the shorter loop at each self-intersection is cut) and `--verbose` names the folding edges. At the player's zoom one world unit is about one pixel (a 1400×900u view), which is the scale to judge features at. VERTEX BUDGET (Wes, later on 2026-09-14): the whole document under 1,500 vertices — the first cut had 5.5k, the next largest venue (swamp) has 2.5k and the rest are under 1.5k. Now 1,425 in 109 shapes with the same coastline: every contour is Douglas-Peucker simplified (`TOL` — coast 22u, apron 32, meadow 40, forest 50; one unit is about a pixel), the shoreline noise lost its finest component, the apron and shelf are smooth (the coast's own noise sets the wet lip's width), blobs get 5–8 vertices by size, wet halos only under the six big rocks, the meadow edge follows the scallops loosely, and the SHALLOWS ARE UNIONED (Wes's suggestion — `union_rings`, a raster marching-squares union at 24u cells, holes dropped, simplified to 60u; only the bay's inner stack stays a separate blob). The lean pass narrowed the middle channel at the headland to ~150u and three bots clipped the wash there, so wash A now sits on rock B's side (~230u gap). Editor estimate 3:05 at scale 1.12, 5.01 km; bots 3:20–4:12, median 3:50 (1.24× — Wes's rule of thumb is 1.15×), three penalties (two boat contacts at the pin, one in the beat); venue / render / route / sailable suites pass. No props placed yet (`--plant` is opt-in). THE FIVE TIDEPOOL PROPS ARE SHIPPED (2026-09-14, delivery records in their manifest notes): shelf 300, ridge 240 (hand-prepped from a landscape frame; it tapers 1.6× seaward — reroll if it reads as a leaning stack), pinnacle 160, basin 140 (near the donut, accepted), and the wash-rock reef 120, the only one with its own collider (contact hard, contactR 42); the other four take a `coastalgranite` or `tidepool` shape underneath for collider, surf and lee. THE FIVE TIDEPOOL-LIFE PROPS ARE SHIPPED TOO (2026-09-14, round 2): anemone pool 90, urchin pocket 70, sea-star ledge 100, surfgrass channel 120, tidepool garden 130 (oblong at 1.40 against a round brief, accepted) — each owned by one colour, the animals drawn SMALL against the rock (round 1 came back at boat scale: an anemone the size of a cockpit; the rule now is an anemone a thirtieth of the formation, nothing bigger than a dinghy's rudder), all landmarks, contact none. THE FOUR CENTREPIECES ARE SHIPPED (2026-09-14, round 1): tidepool flats 420 (eight pools, a channel, a mussel ledge, kelp wrack, knobs, a sand pocket), surge channel 380 (two unequal life-banded ledges either side of a slot the sea shows through — the delivery painted the slot blue and it was hand-keyed transparent before ingest), cypress crag 320 (the Lone Cypress as a crown on its knob with a pavement, a boulder heap and a whaleback, dry), arch point 480 (a 2.67:1 spur pinched to an isthmus with a guano-capped knob, a blade and a table stack off its end as separate pieces). Wes's 'hero' means a big composed prop placed ONCE, never a venue painting. FIVE OF THE NINE CALIFORNIA ROCKS ARE SHIPPED (2026-09-14): boulder egg 44 (1.23 against a round brief, accepted, the one to reroll if it looks tipped), boulder pile 110 (fourteen stones plus three loose), whaleback 120, pavement 160 (1.18, an oblong patch), fin 140 (Wes's file said blade — 'the blade is actually the fin'); dry, lichen only, no bands, scrub in the joints; landmarks, contact none. The BLADE (150, 3:1, accepted 'not strictly orthographic') and TABLE (130, 1.31, sides visible, accepted) stacks shipped the same day. The BIRD ROCK (220) and SEA ARCH (180) shipped on their second rounds the same day: the bird rock a plan view now (round 1 was a three-quarter bun; the rim is still thicker on the shaded side, accepted), the arch one spur with a quarter-width neck and transparent notches (round 1 was the arch-point centrepiece byte for byte). EVERY ROCK SLOT IS SHIPPED — 36 entries. A SECOND VEGETATION SET followed the same evening from the gaps the California references showed: lizard tail 26 (the meadow's yellow accent — measured dE 2 from its spec and dE 30 from the meadow tan), coastal thicket 48 (the big dark bush of the forest edge), ghost cypress 90 (the one tree drawn as branches — a bleached skeleton), dune grass 24 (fountains rather than wind-laid, and only dE 19 from the sand; accepted, reroll if it vanishes), cypress grove 220 (four crowns in a two-by-two rather than merged with one dominant; accepted as the coverage prop), plus a driftwood log 36 and kelp wrack 30 for the beaches and the Point Pinos light 96 (a keeper's cottage with the tower through its roof, planRound false — the cottage is 1.33:1 by its brief). All registered, all contact none. The red ice plant landed last (36, leaves red from the tips, no flowers). Four of the small props were RAISED after the at-size sheet — lizard tail 26→40, dune grass 24→34, driftwood 36→54, kelp wrack 30→40 — because at their briefed sizes they were a few pixels and the lizard tail's yellow did not carry; re-baked from the same masters. EVERY OTTER SLOT IS ART: 45 entries, none open. Not built: the building sea breeze, the fog bank, swell damping in kelp, and prop placement. The script overwrites the document — hand edits in editor.html are lost on a re-run, so either edit the script or stop re-running it. WES HAS PLACED 445 PROPS BY HAND (2026-09-14, night) — cypress groves, pines, ice plant, the tidepool rocks, the stacks, the arch point, the crag, the light — so `build_otter.py` MUST NOT be re-run against this file. THE ROCKS ARE HARD NOW (2026-09-14, night): eighteen Otter rock kinds (every tidepool rock, the four centrepieces, the boulders, outcrops, stacks, the sea arch and the light) went from landmark `contact: none` to `contact: hard` with an authored `height` (shelf, flats and pavement 0; basin 2; egg and surge 3; ridge and pile 4; fin 5; whaleback 6; table and sea arch 8; arch point and bird rock 10; crag, blade and the light 12; pinnacle 17) — the collider is the traced paint outline (`js/prop_outlines.js`, the arch point is three rings), the prop takes surf on its weather faces and casts a lee from its height, so NO shape need be laid under a rock any more; the redrock-tower rule above is superseded for this venue and the reef's contactR 42 is the one circle left. The document was re-baked after the flip (sig v3, best 3:10). BOTH ARCHES ARE PASSAGES (2026-09-14, night — Wes: 'make the area under the arches sailable as a small passage… the arch renders over the boat, no transparency'): `otter-centre-arch-point` and `otter-sea-arch` are composites (piers + opaque span, see the Lighthouse Cove collider passage), the passage under the arch point's isthmus is ~85u wide with ~120u of rock overhead at its 3.5x placement, and the sea arch was scaled from 2.74x to 4x (its 16u-per-scale neck run was two beams at 2.74x) for a ~64u passage under ~76u of rock. The bots' ~50u collision diameter fits the arch point and only just the sea arch; the baked ideal path did not change (3:10.7). Under the span the player steers blind on the HUD chip — four seconds fully hidden at 6 kt under the arch point — which is the design, not a bug; `opaque` is the knob if it ever needs to soften. PERFORMANCE PASS (2026-09-14, night — Wes: 'it seems to drag a bit'): measured in his Chrome at 88 ms a frame, a third of it ONE draw — the unioned shallows sprite (r 15,193u, a 1024-px bake) issued as a 30,667-px square every frame; now every world sprite draws only its visible window (drawZoneSprite), the static land is a world tile baked at device scale once per 400u of travel (drawIslandsCached), the lee sampler rejects far casters before any silhouette work, wind-wave cells and wind streaks sample the wind on a cadence, hull discs use a per-polygon edge grid, the swell draws at half resolution and the chart's still layers are cached. After: ~9 ms CPU median, ~15 ms wall. The remaining stutter is the bots' replanning (single updateBoat calls of ~50 ms). ROUND 2 (the headland was still 'jumpy'): the goal chip's route recomputed twice a second at up to 80 ms — its string-pull now gallops instead of scanning from the far end, and the segment-clearance test keeps per-edge boxes; and the surf's dry-edge tables (built the first frame a shape enters view, the coast's alone 200k polygon tests) now go through the collision grid's point test and are built at most three a frame.
