...
(async () => {
    ...
    for (let i = 0; i < TRIALS && gotStall < NSTALL; i++) {
        ...
        for (const ep of eps.done) {
            ...
            console.log('t | d brg | kt hdg twa | arm reb sw wr bank | rul eBrg out xBrg | defl nav | liv nr');
            let last = null;
            for (const r of ep.rows) {
                ...
                console.log(`${r.t} | ${r.d} ${r.brg} | ${r.kt} ${r.hdg} ${r.twa} | ${r.arm} ${r.reb} ${r.sw} ${r.wr} ${r.bank} | ${r.rul} ${r.eBrg} ${r.out} ${r.xBrg} | ${r.defl} (${r.navx},${r.navy}) | ${r.liv} ${r.nr}`);
                last = { ...r, pt: r.t };
            }
        }
    }
