...
//
// node _rub_probe.js <trials> <seed0> <tree> <label> [venue]  (venue-generic copy of _rub_probe)
...
const VENUE = process.argv[6] || 'bay';
...
(async () => {
    ...
    if (LABEL) {
        fs.writeFileSync(path.join(__dirname, `rub_${LABEL}.json`), JSON.stringify(all));
        console.log(`\nwrote rub_${LABEL}.json`);
    }
})();
