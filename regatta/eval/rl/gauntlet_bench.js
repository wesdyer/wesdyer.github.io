// Redrock Reservoir (redrock) fleet profiler: full fleet, cutoff raised to 900,
...
(async () => {
    ...
    for (let i = 0; i < TRIALS; i++) {
        ...
        const r = await page.evaluate(async (seed) => {
            ...
            const pl = state.boats.find(b => b.isPlayer); pl.x = -6000; pl.y = 6000;
    ...
    fs.writeFileSync(path.join(__dirname, 'gauntlet_bench_' + LABEL + '.json'), JSON.stringify(out));
    console.log('saved gauntlet_bench_' + LABEL + '.json  nLegs', out[0].nLegs, 'legLens', out[0].legLens.join(','));
    await browser.close();
})();
