...
(async () => {
    ...
    for (let i = 0; i < TRIALS; i++) {
        ...
        const r = await page.evaluate(async (seed) => {
            ...
            const mkSeg = () => ({ t: 0, d: 0, slow: 0, grind: 0,
                ...
                exAvNLat: 0, exAvNFloe: 0, exAvNProx: 0, exAvNLatch: 0, exAvNTrue: 0,
                tRec: 0, tTurn: 0, tAvoid: 0, tOffrt: 0, tSail: 0, tPark: 0,
                avDevSum: 0, avDevN: 0,
            ...
            for (let it = 0; it < 60 * 940; it++) {
                ...
                for (let k = 0; k < bots.length; k++) {
                    ...
                    g[cls] += excessW;
                    g['t' + cls.slice(2)] += 1;
                    if (s.wOdo < 8) g.tPark += 1;   // near-stationary second
                    if (cls === 'exAvoid') {
                        ...
                        if (hadBoat && hadStatic) g.exAvBoth += excessW;
                            ...
                            g.exAvNone += excessW;
                            // sub-bin the mystery: what is the boat actually
                            // bending around when nothing sits dead ahead?
                            let lat = false;   // blockage on a +-30deg ray (probe was too narrow)
                            ...
                            for (const isl of (state.course.islands || [])) {
                                ...
                                if (Math.hypot(b.x - isl.x, b.y - isl.y) < isl.radius + 250) { floeNear = true; break; }
                            }
                            let prox = false;
                            for (const o of state.boats) {
                                if (o === b || o.raceState.finished) continue;
                                if ((o.x - b.x) ** 2 + (o.y - b.y) ** 2 < 300 * 300) { prox = true; break; }
                            }
                            const latch = !!(c && c.avoidanceCommitTimer > 0);
                            if (lat) g.exAvNLat += excessW;
                            else if (floeNear) g.exAvNFloe += excessW;
                            else if (prox) g.exAvNProx += excessW;
    ...
    for (const ph of ['transit', 'ret']) {
        ...
        console.log(`  avoid-none detail: lat-static ${S('exAvNLat').toFixed(0)} | floe<250u ${S('exAvNFloe').toFixed(0)} | boat<300u ${S('exAvNProx').toFixed(0)} | latch ${S('exAvNLatch').toFixed(0)} | true-none ${S('exAvNTrue').toFixed(0)}`);
        console.log(`  SECONDS by class: rec ${S('tRec').toFixed(0)} | turn ${S('tTurn').toFixed(0)} | avoid ${S('tAvoid').toFixed(0)} | offrt ${S('tOffrt').toFixed(0)} | sail ${S('tSail').toFixed(0)}   park(<8u/s) ${S('tPark').toFixed(0)}s`);
