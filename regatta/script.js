...
const ALL_START_STRATEGIES = [
    "Time-on-Distance Start", "Wind-Shift Anticipation Start", "Gap Sniper",
    "Port-Tack Flyer", "Chaos Gambit", "Second-Row Slingshot",
    "Pin-End (Committee Boat) Start", "Luff & Kill", "High-Lane Builder",
    "Mid-Line Safety Start", "Leebow Trap", "Bail-Out & Reset"
];
...
class Boat {
    constructor(id, isPlayer, startX, startY, name="USA", config=null) {
        ...
        // Personality Stats
        if (config) {
             // Standardized AI Stats (No Personality)
             this.stats = {
                 boatHandling: 4,
                 windSense: 4,
                 positioning: 4,
...
function resetGame() {
    ...
    for (let i = 0; i < opponents.length; i++) {
        ...
        // Pick Strategy (Randomized from global pool)
        ai.ai.startStrategy = ALL_START_STRATEGIES[Math.floor(Math.random() * ALL_START_STRATEGIES.length)];
