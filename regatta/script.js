...
// AI Configuration
const AI_CONFIG = [
    { name: 'Bixby', creature: 'Otter', hull: '#0046ff', spinnaker: '#FFD400', sail: '#FFFFFF', cockpit: '#C9CCD6', boatHandling: 4, windSense: 5, positioning: 4, aggression: 2, composure: 5 , startStrategies: ["Time-on-Distance Start", "Wind-Shift Anticipation Start"], personality: "Relaxed veteran who instinctively finds the right place." },
    { name: 'Skim', creature: 'Flying Fish', hull: '#8FD3FF', spinnaker: '#FF2D95', sail: '#FFFFFF', cockpit: '#AEB4BF', boatHandling: 5, windSense: 3, positioning: 3, aggression: 4, composure: 3 , startStrategies: ["Gap Sniper", "Port-Tack Flyer"], personality: "Flashy opportunist thriving on speed and lane changes." },
    { name: 'Wobble', creature: 'Platypus', hull: '#FF8C1A', spinnaker: '#00E5FF', sail: '#FFFFFF', cockpit: '#B0B0B0', boatHandling: 3, windSense: 3, positioning: 2, aggression: 4, composure: 2 , startStrategies: ["Chaos Gambit", "Second-Row Slingshot"], personality: "Awkward but effective when chaos breaks out." },
    { name: 'Pinch', creature: 'Lobster', hull: '#E10600', spinnaker: '#FFFFFF', sail: '#FFFFFF', cockpit: '#5A5A5A', boatHandling: 4, windSense: 2, positioning: 5, aggression: 5, composure: 3 , startStrategies: ["Pin-End (Committee Boat) Start", "Luff & Kill"], personality: "Aggressive specialist in brutal starting-line pressure." },
    { name: 'Bruce', creature: 'Great White', hull: '#121212', spinnaker: '#ff0606', sail: '#FFFFFF', cockpit: '#3A3A3A', boatHandling: 4, windSense: 3, positioning: 4, aggression: 5, composure: 4 , startStrategies: ["Pin-End (Committee Boat) Start", "High-Lane Builder"], personality: "Cold, dominant, and relentless; forces others to react." },
    { name: 'Strut', creature: 'Flamingo', hull: '#FF4F9A', spinnaker: '#000000', sail: '#FFFFFF', cockpit: '#B0BEC5', boatHandling: 4, windSense: 3, positioning: 3, aggression: 4, composure: 3 , startStrategies: ["Gap Sniper", "High-Lane Builder"], personality: "Stylish, confident, sailing with flair and daring." },
    { name: 'Gasket', creature: 'Beaver', hull: '#FFE600', spinnaker: '#000000', sail: '#000000', cockpit: '#C4BEB2', boatHandling: 3, windSense: 4, positioning: 5, aggression: 2, composure: 4 , startStrategies: ["Mid-Line Safety Start", "Leebow Trap"], personality: "Methodical builder of advantages that are hard to dismantle." },
    { name: 'Chomp', creature: 'Alligator', hull: '#2ECC71', spinnaker: '#F4C27A', sail: '#000000', cockpit: '#C1B58A', boatHandling: 4, windSense: 3, positioning: 4, aggression: 4, composure: 4 , startStrategies: ["Leebow Trap", "Port-Tack Flyer"], personality: "Patient and dangerous, striking when least expected." },
    { name: 'Whiskers', creature: 'Walrus', hull: '#C49A6C', spinnaker: '#8E0038', sail: '#FFFFFF', cockpit: '#ddd3c9', boatHandling: 3, windSense: 3, positioning: 4, aggression: 2, composure: 5 , startStrategies: ["Mid-Line Safety Start", "High-Lane Builder"], personality: "Big, steady, immovable; excels in heavy conditions." },
    { name: 'Vex', creature: 'Lizard', hull: '#0fe367', spinnaker: '#D9D9D9', sail: '#FFFFFF', cockpit: '#D0D0D0', boatHandling: 4, windSense: 4, positioning: 3, aggression: 3, composure: 3 , startStrategies: ["Gap Sniper", "Leebow Trap"], personality: "Slippery and cunning, exploiting tiny mistakes." },
    ...
    { name: 'Ripple', creature: 'Dolphin', hull: '#00B3FF', spinnaker: '#FF6F00', sail: '#FFFFFF', cockpit: '#B8C6D1', boatHandling: 5, windSense: 3, positioning: 3, aggression: 4, composure: 3 , startStrategies: ["Gap Sniper", "Time-on-Distance Start"], personality: "Cheerful speedster seeking clean lanes and open water." },
    ...
    { name: 'Pearl', creature: 'Oyster', hull: '#C7A6FF', spinnaker: '#2E2E2E', sail: '#FFFFFF', cockpit: '#CFCFD4', boatHandling: 3, windSense: 5, positioning: 4, aggression: 1, composure: 5 , startStrategies: ["Bail-Out & Reset", "Wind-Shift Anticipation Start"], personality: "Quiet, patient; wins by waiting out mistakes." },
    { name: 'Bluff', creature: 'Polar Bear', hull: '#FFFFFF', spinnaker: '#00AEEF', sail: '#FFFFFF', cockpit: '#BFC6CC', boatHandling: 3, windSense: 3, positioning: 4, aggression: 2, composure: 5 , startStrategies: ["High-Lane Builder", "Mid-Line Safety Start"], personality: "Calm, imposing, unbothered; dares others to blink first." },
    ...
    { name: 'Pulse', creature: 'Tree Frog', hull: '#00FF6A', spinnaker: '#7A00FF', sail: '#FFFFFF', cockpit: '#C9CCD6', boatHandling: 4, windSense: 3, positioning: 3, aggression: 5, composure: 3 , startStrategies: ["Time-on-Distance Start", "Chaos Gambit"], personality: "Hyper-alert, explosive; thrives on quick reactions." }
];
...
function updateAITrim(boat, optimalSailAngle, dt) {
    ...
    let target = boat.ai.currentTrimTarget;

    // Override for Forced Luff (Immediate)
    if (boat.ai.forcedLuff > 0) {
        // Luffing: Open sail to spill wind
        // Target: optimal + luff amount (up to 90 degrees more)
    ...
    let speed = 0.2 + (handling - 1) * 0.45;

    // Luffing should be faster (letting out is easy)
    if (boat.ai.forcedLuff > 0) speed *= 3.0;
...
function updateAI(boat, dt) {
    ...
    if (boat.raceState.finished) {
        // Target nearest boundary point (exit course)
        const b = state.course.boundary;
        if (b) {
            const dx = boat.x - b.x;
            const dy = boat.y - b.y;
        ...
        // Override targetAngle logic below by setting a phantom target?
        // Actually we calculate angle from targetX,Y below.
        // Start strategy returns angle too, but we might want to respect x,y for smooth steering.
        // Let's just use the computed X,Y.
    } else {
        // Racing Logic
        ...
        if (marks && marks.length >= 4) {
            ...
            if (boat.raceState.isRounding) {
                ...
                const ux = vmx / distM;
                const uy = vmy / distM;
                const offset = 80 + (5 - positioning) * 20; // Wide turn
    ...
    if (isRacing || isPrestart) {
        ...
        if (boat.ai.stuckTimer > timeThreshold && !boat.ai.recoveryMode) {
             ...
             // Try to reach (90 degrees / 1.57 rad)
    ...
    // 3. Sailing Logic (Tacking/Gybing)
    ...
    // Clamp Steering Bias to prevent spinning
    ...
    // Override Speed Limit for Recovery
    if (boat.ai.recoveryMode) speedLimit = 1.0;

    // Apply Speed Limit to Trim
    if (speedLimit < 0.9) {
    ...
    // Steering Physics (same as before)
