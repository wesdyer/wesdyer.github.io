...
function updateLeaderboard() {
    ...
    if (UI.lbRows) {
        ...
        sorted.forEach((boat, index) => {
            ...
            // Visual Identifier (Star or Portrait)
            let visualId;
            if (boat.isPlayer) {
                visualId = document.createElement('div');
                visualId.className = "w-8 h-8 mr-2 flex items-center justify-center";
                visualId.innerHTML = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="${settings.hullColor}" class="w-6 h-6 drop-shadow-sm">
                ...
                visualId.className = "w-8 h-8 rounded-full mr-2 object-cover border-2 bg-slate-800";
                visualId.style.borderColor = boat.colors.hull;
            }
