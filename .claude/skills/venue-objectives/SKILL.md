...
description: Design, then build, one regatta venue's character objectives and its three animals (unlocks.js rows, feats, wildlife config, tests, venue-roster.md). Use when Wes says "let's do <venue> next" or asks for a venue's achievements/unlocks/objectives.
---

# Venue objectives — design first, then build

Done so far: Lighthouse Cove (`bay`), Sailing School (`pond`), Stillwater Lake (`lake`),
Pearl Lagoon (`lagoon`), Gatorgrass Bayou (`swamp`). Remaining: river, ocean, redrock,
glowtide, arctic, otter, flats, volcanic.
...
- **Three animal types per venue.** At least one animal is the subject of an objective
  (Explorer or Wildlife); the rest can be scenery.
...
5. **Pick the three animals.** Choose real species of the place and give each one a behaviour
...
7. **Run:** `test_<venue>`, `test_unlocks`, `test_pages`, and the other venue tests
